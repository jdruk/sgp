...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { city: @local.city, country: @local.country, name: @local.name, state: @local.state, zip_code: @local.zip_code }
    end
  ...
  test "should update local" do
    patch :update, id: @local, local: { city: @local.city, country: @local.country, name: @local.name, state: @local.state, zip_code: @local.zip_code }
    assert_redirected_to local_path(assigns(:local))
  end
